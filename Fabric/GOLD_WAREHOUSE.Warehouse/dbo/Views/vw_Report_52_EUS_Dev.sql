...
SELECT  c.InventSiteId AS 'Kode Outlet', 
		...
		Case
			 when DATEDIFF(month,f.ValidFrom,a.InvoiceDate) <= 12 then 'Sales Trainee' 
			 when DATEDIFF(month,f.ValidFrom,a.InvoiceDate) > 12 and DATEDIFF(month,f.ValidFrom,a.InvoiceDate) <= 24 then 'Junior Sales'
			 when DATEDIFF(month,f.ValidFrom,a.InvoiceDate) > 25 and DATEDIFF(month,f.ValidFrom,a.InvoiceDate) <= 36 then 'Sales'
			 when DATEDIFF(month,f.ValidFrom,a.InvoiceDate) > 36 then 'Senior Sales' 
...
LEFT OUTER JOIN dbo.vw_Kacab AS pwa on pwa.PersonnelNumber = c.ZSupervisor
LEFT OUTER  JOIN dbo.Report_53_BM AS l	ON l.SITE = c.InventSiteId and Left(l.RUNNINGDATE,6) = Left(convert(char,a.InvoiceDate,112),6)

-- Auto Generated (Do not modify) E8E99BCC7CDED115DC1FC8ACD768D148984373DA323217B2A688122DDFCBE693
CREATE view vw_report_performace_bengkel_25092025
as
...
from FL_T
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_T.Scenario = RF.Scenario and ReportCode = 'TU'
group by 
date,[Kode Outlet],RF.Code
union all
select date,[Kode Outlet],RF.Code,sum(Value) value,'Target - Unit CV/LCV' as src
from FL_T
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_T.Scenario = RF.Scenario and ReportCode = 'TUD' and FL_T.MajorGroup = RF.MajorGroupId
group by 
date,[Kode Outlet],RF.Code
union all
select date,[Kode Outlet],RF.Code,sum(Value) value,'Target - Revenue' as src
from FL_T
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_T.Scenario = RF.Scenario and ReportCode = 'TR'
group by 
date,[Kode Outlet],RF.Code
...
from FL_T
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_T.Scenario = RF.Scenario and ReportCode = 'TRD' and FL_T.MajorGroup = RF.MajorGroupId
group by 
date,[Kode Outlet],RF.Code
-- Achievement Unit
union all
...
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on ReportCode = 'TUP' 
...
select FL_T.date,[Kode Outlet],RF.Code,IIF(sum(cast(FL_T.Value as decimal (8,2)))=0,0,(cast(sum(UST.Value)as decimal (8,2))/cast(sum(FL_T.Value)as decimal (8,2))))*100 value,'Target - Achievement Unit ' as src
from unit_served_target FL_T
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on ReportCode = 'TUPD' and FL_T.MajorGroup = RF.MajorGroupId
join unit_served_total_by_MajorGroup UST on FL_T.[Kode Outlet] = UST.InventSiteId and LEFT(FL_T.date,7) = InvoiceDate and FL_T.MajorGroup = UST.MajorGroup
group by 
FL_T.date,[Kode Outlet],RF.Code
-- Achievement Revenue
...
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on ReportCode = 'TRP'
...
select FL_T.date,[Kode Outlet],RF.Code,IIF(sum(FL_T.Value)=0,0,(sum(RST.Value)/sum(FL_T.Value)))*100 value,'Target - Achievement Revenue' as src
from revenue_target FL_T
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on ReportCode = 'TRPD' and FL_T.MajorGroup = RF.MajorGroupId
join revenue_served_total_by_MajorGroup RST on FL_T.[Kode Outlet] = RST.InventSiteId and LEFT(FL_T.date,7) = InvoiceDate and FL_T.MajorGroup = RST.MajorGroup
group by 
FL_T.date,[Kode Outlet],RF.Code
...
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on ReportCode = 'JHK'
...
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on ReportCode = 'JCSK'
group by Dates,Outlet,RF.Code
...
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on ReportCode = 'RPNK'
group by Dates,Outlet,RF.Code
...
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on ReportCode = 'RPNKD'
group by Dates,Outlet,RF.Code
...
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL.Brand = RF.Brand and FL.MajorGroup = RF.MajorGroupId and FL.MinorGroupId = RF.MinorGroupId and ReportCode = 'RPU'
...
select cast(cast(year(InvoiceDate)as varchar(255))+'/'+cast(month(InvoiceDate) as varchar(255))+'/'+'01' as date) date,InventSiteId,RF.Code report_id,IIF(sum(qty)=0,0,sum(revenue)/sum(qty)) value,'revenue per unit' as src
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL.Brand = RF.Brand and FL.MajorGroup = RF.MajorGroupId and ReportCode = 'RPUH'
group by cast(cast(year(InvoiceDate)as varchar(255))+'/'+cast(month(InvoiceDate) as varchar(255))+'/'+'01' as date),FL.InventSiteId,RF.Code
-- Revenue per unit Brand total
union all
select cast(cast(year(InvoiceDate)as varchar(255))+'/'+cast(month(InvoiceDate) as varchar(255))+'/'+'01' as date) date,InventSiteId,RF.Code report_id,IIF(sum(qty)=0,0,sum(revenue)/sum(qty)) value,'revenue per unit' as src
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL.Brand = RF.Brand and ReportCode = 'RPU' and IsTotal = 'BT'
group by cast(cast(year(InvoiceDate)as varchar(255))+'/'+cast(month(InvoiceDate) as varchar(255))+'/'+'01' as date),FL.InventSiteId,RF.Code
-- Revenue per unit grand total
union all
select cast(cast(year(InvoiceDate)as varchar(255))+'/'+cast(month(InvoiceDate) as varchar(255))+'/'+'01' as date) date,InventSiteId,RF.Code report_id,IIF(sum(qty)=0,0,sum(revenue)/sum(qty)) value,'revenue per unit' as src
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on ReportCode = 'RPU' and IsTotal = 'GT'
group by cast(cast(year(InvoiceDate)as varchar(255))+'/'+cast(month(InvoiceDate) as varchar(255))+'/'+'01' as date),FL.InventSiteId,RF.Code
...
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL.Brand = RF.Brand and FL.MajorGroup = RF.MajorGroupId and FL.MinorGroupId = RF.MinorGroupId and ReportCode = 'US'
...
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat]  RF on FL.Brand = RF.Brand and FL.MajorGroup = RF.MajorGroupId and ReportCode = 'USH'
group by FL.InvoiceDate,FL.InventSiteId,RF.Code
-- Brand total
union all
select FL.InvoiceDate,FL.InventSiteId,RF.Code report_id,sum(qty) value,'unit served - qty' as src
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat]  RF on FL.Brand = RF.Brand and IsTotal = 'BT' and ReportCode = 'US'
group by FL.InvoiceDate,FL.InventSiteId,RF.Code
-- Grand Total
union all
select FL.InvoiceDate,FL.InventSiteId,RF.Code report_id,sum(qty) value,'unit served - qty' as src
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat]  RF on IsTotal = 'GT' and ReportCode = 'US'
group by FL.InvoiceDate,FL.InventSiteId,RF.Code
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(qty) value,'unit served - qty' as src
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL.Stall_BIB = RF.Stall_BIB and FL.Brand = RF.Brand and FL.MajorGroup = RF.MajorGroupId and FL.MinorGroupId = RF.MinorGroupId and ReportCode = 'USB'
group by FL.InvoiceDate,FL.InventSiteId,RF.Code
-- MajorGroup Total BIB
union all
select FL.InvoiceDate,FL.InventSiteId,RF.Code report_id,sum(qty) value,'unit served - qty' as src
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat]  RF on FL.Stall_BIB = RF.Stall_BIB and FL.Brand = RF.Brand and FL.MajorGroup = RF.MajorGroupId and ReportCode = 'USBH'
group by FL.InvoiceDate,FL.InventSiteId,RF.Code
-- Brand Total BIB
union all
select FL.InvoiceDate,FL.InventSiteId,RF.Code report_id,sum(qty) value,'unit served - qty' as src
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat]  RF on FL.Stall_BIB = RF.Stall_BIB and FL.Brand = RF.Brand and IsTotal = 'BT' and ReportCode = 'USB'
group by FL.InvoiceDate,FL.InventSiteId,RF.Code
-- Grand Total BIB
union all
select FL.InvoiceDate,FL.InventSiteId,RF.Code report_id,sum(qty) value,'unit served - qty' as src
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat]  RF on FL.Stall_BIB = RF.Stall_BIB and IsTotal = 'GT' and ReportCode = 'USB'
group by FL.InvoiceDate,FL.InventSiteId,RF.Code
-- Unit Served Service Contract
union all
select FL.InvoiceDate,FL.InventSiteId,RF.Code report_id,sum(qty) value,'unit served - qty' as src
from FL_CON FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat]  RF on ReportCode = 'USSC'
group by FL.InvoiceDate,FL.InventSiteId,RF.Code
...
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL.Brand = RF.Brand and FL.MajorGroup = RF.MajorGroupId and FL.MinorGroupId = RF.MinorGroupId and ReportCode = 'USR'
group by FL.InvoiceDate,FL.InventSiteId,RF.Code
-- MajorGroup total
union all
select FL.InvoiceDate,FL.InventSiteId,RF.Code report_id,sum(revenue) value,'revenue - qty' as src
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat]  RF on FL.Brand = RF.Brand and FL.MajorGroup = RF.MajorGroupId and ReportCode = 'USRH'
group by FL.InvoiceDate,FL.InventSiteId,RF.Code
-- Brand total
union all
select FL.InvoiceDate,FL.InventSiteId,RF.Code report_id,sum(revenue) value,'revenue - qty' as src
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat]  RF on FL.Brand = RF.Brand and IsTotal = 'BT' and ReportCode = 'USR'
group by FL.InvoiceDate,FL.InventSiteId,RF.Code
-- Grand Total
union all
select FL.InvoiceDate,FL.InventSiteId,RF.Code report_id,sum(revenue) value,'revenue - qty' as src
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat]  RF on IsTotal = 'GT' and ReportCode = 'USR'
group by FL.InvoiceDate,FL.InventSiteId,RF.Code
-- revenue BIB
union all
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(revenue) value,'revenue - qty' as src
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL.Stall_BIB = RF.Stall_BIB and FL.Brand = RF.Brand and FL.MajorGroup = RF.MajorGroupId and FL.MinorGroupId = RF.MinorGroupId and ReportCode = 'USRB'
group by FL.InvoiceDate,FL.InventSiteId,RF.Code
-- MajorGroup total BIB
union all
select FL.InvoiceDate,FL.InventSiteId,RF.Code report_id,sum(revenue) value,'revenue - qty' as src
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat]  RF on FL.Stall_BIB = RF.Stall_BIB and FL.Brand = RF.Brand and FL.MajorGroup = RF.MajorGroupId and ReportCode = 'USRBH'
group by FL.InvoiceDate,FL.InventSiteId,RF.Code
-- Brand total BIB
union all
select FL.InvoiceDate,FL.InventSiteId,RF.Code report_id,sum(revenue) value,'revenue - qty' as src
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat]  RF on FL.Stall_BIB = RF.Stall_BIB and FL.Brand = RF.Brand and IsTotal = 'BT' and ReportCode = 'USRB'
group by FL.InvoiceDate,FL.InventSiteId,RF.Code
-- Grand Total BIB
union all
select FL.InvoiceDate,FL.InventSiteId,RF.Code report_id,sum(revenue) value,'revenue - qty' as src
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat]  RF on FL.Stall_BIB = RF.Stall_BIB and IsTotal = 'GT' and ReportCode = 'USRB'
group by FL.InvoiceDate,FL.InventSiteId,RF.Code
-- Revenue Service Contract
union all
select FL.InvoiceDate,FL.InventSiteId,RF.Code report_id,sum(revenue) value,'revenue - qty' as src
from FL_CON FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat]  RF on ReportCode = 'RSC'
group by FL.InvoiceDate,FL.InventSiteId,RF.Code
...
from FL_JL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_JL.Brand = RF.Brand and FL_JL.MajorGroup = RF.MajorGroupId and FL_JL.MinorGroupId = RF.MinorGroupId and FL_JL.Joblist = RF.Joblist 
and ReportCode = 'USJL'
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(qty) value,'Unit Served by Joblist - qty' as src
from FL_JL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_JL.Brand = RF.Brand and FL_JL.MajorGroup = RF.MajorGroupId and FL_JL.MinorGroupId = RF.MinorGroupId
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(qty) value,'Unit Served by Joblist - qty' as src
from FL_JL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_JL.Brand = RF.Brand and FL_JL.MajorGroup = RF.MajorGroupId 
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(qty) value,'Unit Served by Joblist - qty' as src
from FL_JL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_JL.Brand = RF.Brand and FL_JL.MajorGroup = RF.MajorGroupId and FL_JL.Joblist = RF.Joblist
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(qty) value,'Unit Served by Joblist - qty' as src
from FL_JL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_JL.Brand = RF.Brand
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(qty) value,'Unit Served by Joblist - qty' as src
from FL_JL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_JL.Brand = RF.Brand and FL_JL.Joblist = RF.Joblist
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(qty) value,'Unit Served by Joblist - qty' as src
from FL_JL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on IsTotal = 'GT' and ReportCode = 'USJLH'
group by FL_JL.InvoiceDate,FL_JL.InventSiteId,RF.Code
-- unit served Joblist total
union all
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(qty) value,'Unit Served by Joblist - qty' as src
from FL_JL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_JL.Joblist = RF.Joblist and IsTotal = 'GT' and ReportCode = 'USJL'
group by FL_JL.InvoiceDate,FL_JL.InventSiteId,RF.Code
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(qty) value,'Unit Served by Joblist - qty' as src
from FL_JL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_JL.Stall_BIB = RF.Stall_BIB and  FL_JL.Brand = RF.Brand and FL_JL.MajorGroup = RF.MajorGroupId and FL_JL.MinorGroupId = RF.MinorGroupId and FL_JL.Joblist = RF.Joblist 
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(qty) value,'Unit Served by Joblist - qty' as src
from FL_JL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_JL.Stall_BIB = RF.Stall_BIB and  FL_JL.Brand = RF.Brand and FL_JL.MajorGroup = RF.MajorGroupId and FL_JL.MinorGroupId = RF.MinorGroupId 
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(qty) value,'Unit Served by Joblist - qty' as src
from FL_JL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_JL.Stall_BIB = RF.Stall_BIB and  FL_JL.Brand = RF.Brand and FL_JL.MajorGroup = RF.MajorGroupId
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(qty) value,'Unit Served by Joblist - qty' as src
from FL_JL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_JL.Stall_BIB = RF.Stall_BIB and  FL_JL.Brand = RF.Brand and FL_JL.MajorGroup = RF.MajorGroupId and FL_JL.Joblist = RF.Joblist
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(qty) value,'Unit Served by Joblist - qty' as src
from FL_JL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_JL.Stall_BIB = RF.Stall_BIB and FL_JL.Brand = RF.Brand
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(qty) value,'Unit Served by Joblist - qty' as src
from FL_JL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_JL.Stall_BIB = RF.Stall_BIB and  FL_JL.Brand = RF.Brand and FL_JL.Joblist = RF.Joblist
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(qty) value,'Unit Served by Joblist - qty' as src
from FL_JL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_JL.Stall_BIB = RF.Stall_BIB and IsTotal = 'GT' and ReportCode = 'USJLBH'
group by FL_JL.InvoiceDate,FL_JL.InventSiteId,RF.Code
-- unit served Joblist total BIB
union all
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(qty) value,'Unit Served by Joblist - qty' as src
from FL_JL
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_JL.Stall_BIB = RF.Stall_BIB and FL_JL.Joblist = RF.Joblist and IsTotal = 'GT' and ReportCode = 'USJLB'
group by FL_JL.InvoiceDate,FL_JL.InventSiteId,RF.Code
...
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_CAT.Brand = RF.Brand and FL_CAT.MajorGroup = RF.MajorGroupId and FL_CAT.MinorGroupId = RF.MinorGroupId and FL_CAT.Category_name = RF.CategoryName 
and ReportCode = 'REV'
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(revenue) value,'revenue per category - revenue' as src
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_CAT.Brand = RF.Brand and FL_CAT.MajorGroup = RF.MajorGroupId and FL_CAT.MinorGroupId = RF.MinorGroupId 
and ReportCode = 'REVH'
group by FL_CAT.InvoiceDate,FL_CAT.InventSiteId,RF.Code
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(revenue) value,'revenue per category - revenue' as src
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_CAT.Brand = RF.Brand and FL_CAT.MajorGroup = RF.MajorGroupId and FL_CAT.Category_name = RF.CategoryName 
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(revenue) value,'revenue per category - revenue' as src
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_CAT.Brand = RF.Brand and FL_CAT.MajorGroup = RF.MajorGroupId
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(revenue) value,'revenue per category - revenue' as src
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_CAT.Brand = RF.Brand and FL_CAT.Category_name = RF.CategoryName 
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(revenue) value,'revenue per category - revenue' as src
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_CAT.Brand = RF.Brand
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(revenue) value,'revenue per category - revenue' as src
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_CAT.Category_name = RF.CategoryName  and IsTotal = 'GT' and ReportCode = 'REV'
group by FL_CAT.InvoiceDate,FL_CAT.InventSiteId,RF.Code
-- revenue by category header grand total
union all
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(revenue) value,'revenue per category - revenue' as src
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on IsTotal = 'GT' and ReportCode = 'REVH'
group by FL_CAT.InvoiceDate,FL_CAT.InventSiteId,RF.Code
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(revenue) value,'revenue per category - revenue' as src
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_CAT.Stall_BIB = RF.Stall_BIB and FL_CAT.Brand = RF.Brand and FL_CAT.MajorGroup = RF.MajorGroupId and FL_CAT.MinorGroupId = RF.MinorGroupId and FL_CAT.Category_name = RF.CategoryName 
and ReportCode = 'REVB'
group by FL_CAT.InvoiceDate,FL_CAT.InventSiteId,RF.Code
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(revenue) value,'revenue per category - revenue' as src
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_CAT.Stall_BIB = RF.Stall_BIB and FL_CAT.Brand = RF.Brand and FL_CAT.MajorGroup = RF.MajorGroupId and FL_CAT.MinorGroupId = RF.MinorGroupId 
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(revenue) value,'revenue per category - revenue' as src
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_CAT.Stall_BIB = RF.Stall_BIB and FL_CAT.Brand = RF.Brand and FL_CAT.MajorGroup = RF.MajorGroupId and FL_CAT.Category_name = RF.CategoryName 
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(revenue) value,'revenue per category - revenue' as src
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_CAT.Stall_BIB = RF.Stall_BIB and FL_CAT.Brand = RF.Brand and FL_CAT.MajorGroup = RF.MajorGroupId
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(revenue) value,'revenue per category - revenue' as src
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_CAT.Stall_BIB = RF.Stall_BIB and FL_CAT.Brand = RF.Brand and FL_CAT.Category_name = RF.CategoryName 
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(revenue) value,'revenue per category - revenue' as src
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_CAT.Stall_BIB = RF.Stall_BIB and FL_CAT.Brand = RF.Brand
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(revenue) value,'revenue per category - revenue' as src
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_CAT.Stall_BIB = RF.Stall_BIB and FL_CAT.Category_name = RF.CategoryName  and IsTotal = 'GT' and ReportCode = 'REVB'
group by FL_CAT.InvoiceDate,FL_CAT.InventSiteId,RF.Code
-- revenue by category header grand total BIB
union all
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(revenue) value,'revenue per category - revenue' as src
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_CAT.Stall_BIB = RF.Stall_BIB and IsTotal = 'GT' and ReportCode = 'REVBH'
group by FL_CAT.InvoiceDate,FL_CAT.InventSiteId,RF.Code
...
--select InvoiceDate date,InventSiteId,RF.Code report_id,sum(revenue) Value,'revenue by Joblist - revenue' as src
--from FL_JL
--join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_JL.Joblist = RF.Joblist and IsTotal = 'GT' and ReportCode = 'USJLR'
--group by FL_JL.InvoiceDate,FL_JL.InventSiteId,RF.Code
----revenue Joblist BIB
--union all
--select InvoiceDate date,InventSiteId,RF.Code report_id,sum(revenue) Value,'revenue by Joblist - revenue' as src
...
--from FL_JL
--join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_JL.Stall_BIB = RF.Stall_BIB and FL_JL.Joblist = RF.Joblist and IsTotal = 'GT' and ReportCode = 'USJLRB'
--group by FL_JL.InvoiceDate,FL_JL.InventSiteId,RF.Code
...
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on 
...
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(Cost) value,'Cost per category' as src
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on ReportCode = 'CPCH'
group by FL_CAT.InvoiceDate,FL_CAT.InventSiteId,RF.Code
...
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_CAT.Stall_BIB = RF.Stall_BIB and FL_CAT.Category_name = RF.CategoryName and ReportCode = 'CPCB'
group by FL_CAT.InvoiceDate,FL_CAT.InventSiteId,RF.Code
-- Cost per category bib header
union all
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(Cost) value,'Cost per category bib' as src
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on FL_CAT.Stall_BIB = RF.Stall_BIB and ReportCode = 'CPCBH'
group by FL_CAT.InvoiceDate,FL_CAT.InventSiteId,RF.Code
...
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on RF.ReportCode = 'PM'
...
from unit_served_total ust
join data_outlet do on ust.InvoiceDate = do.InvoiceDate and ust.InventSiteId = do.Outlet 
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on RF.ReportCode = 'PBE'
...
from unit_served_total ust
join data_outlet do on ust.InvoiceDate = do.InvoiceDate and ust.InventSiteId = do.Outlet 
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on RF.ReportCode = 'PBI'

-- REVENUE PART DIRECT
union all
select InvoiceDate date,InventSiteId,RF.Code report_id,sum(revenue) value,'Revenue part direct' as src
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on ReportCode = 'RPD'
...
from FL_CAT
join SILVER_WAREHOUSE.[dbo].[ReportFormat] RF on ReportCode = 'RPB'
...
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat]  RF on ReportCode = 'GP'
group by FL.InvoiceDate,FL.InventSiteId,RF.Code
...
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat]  RF on ReportCode = 'GPM'
...
select FL.InvoiceDate,FL.InventSiteId,RF.Code report_id,(sum(revenue)-sum(Cost)) value,'Gross Profit' as src
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat]  RF on ReportCode = 'GPB'
...
select (left(InvoiceDate,7)+'-01') InvoiceDate,FL.InventSiteId,RF.Code report_id,IIF(sum(revenue)=0,0,(sum(revenue)-sum(Cost))/sum(revenue))*100 value,'Gross Profit Margin' as src
from FL
join SILVER_WAREHOUSE.[dbo].[ReportFormat]  RF on ReportCode = 'GPMB'
where FL.Stall_BIB = 'BIB'
group by (left(InvoiceDate,7)+'-01'),FL.InventSiteId,RF.Code
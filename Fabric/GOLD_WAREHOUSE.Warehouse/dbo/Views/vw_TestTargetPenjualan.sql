...
CREATE VIEW [dbo].[vw_TestTargetPenjualan]
AS
with TargetSaleslatest as ( 
SELECT *, ROW_NUMBER() OVER (PARTITION BY SiteCode, YearPeriod, MonthPeriod, FuncAreaId ORDER BY CreatedOn DESC) as rn from TargetSales)
...
select  c.DealerCategory, c.DealerCode as dataAreaId, c.DealerName as ZDealerAfterSales, c.OutletCode as Site, c.OutletName as Name, c.Area, 
	   case when MonthPeriod = 1 then Cast(Cast(YearPeriod as char(4)) + '0101' as Date)  
		...
		MinorGroupId, a.TargetSales Target
from TargetSaleslatest a  
  inner join vw_zInventSite c on c.OutletCode = a.SiteCode
  left join InventItemMinorGroup b on b.ZSeriesTargetPenjualanValue = a.FuncAreaId and left(a.SiteCode,3) = b.dataAreaId and b.MinorGroupId != 'BISON'

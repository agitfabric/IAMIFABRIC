CREATE   PROCEDURE [dbo].[SP_MART_51_OUTLET_PSS]
AS
BEGIN
    SET NOCOUNT ON;
...
DECLARE @running_date varchar(6);
    ...
    SELECT * INTO #mart_51_outlet_pss FROM (
    ...
    -- Hapus baris yang baru diproses (pengganti DEALLOCATE satu per satu)
        DELETE FROM #mart_51_outlet_pss
        WHERE RunningDate = @running_date;
    END

    DROP TABLE #mart_51_outlet_pss;

...
BEGIN
	...
	SET @maxdate = (SELECT ISNULL(
               DATEFROMPARTS(YEAR(MAX([Date])), MONTH(MAX([Date])), 1),
               '1500-01-01'
           )
    FROM Report_42
);
	...
	and lower(InventSiteId) != ''
	and (lower(ParentId) is null or lower(ParentId) = '')
	...
	where lower(ZIAMIAreas) != ''),

	temp as (select * from CaseTable1 a left join ZinventSites1 b on lower(a.INVENTSITEID_CaseTable) = lower(b.Outlet) where lower(b.Outlet) is not null),
	temp2 as (Select a.*,DATEADD(HOUR, 7, GETDATE()) as Last_Update,'' as TipeStall,
	b.WrkCtrId  AS Stall
	from temp a left join SILVER_WAREHOUSE.dbo.WRKCTRTABLE b on lower(a.WRKCTRID_CaseTable) = lower(b.WrkCtrId)
	where lower(SUBSTRING(b.WrkCtrId,7,1)) = 's' and lower(b.WrkCtrId) is not null
	) 

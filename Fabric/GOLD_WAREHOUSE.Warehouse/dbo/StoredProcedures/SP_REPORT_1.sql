...
BEGIN
    ...
    inner join SILVER_WAREHOUSE.dbo.InventItemGroupItem c  
        on lower(c.ItemId) = lower(e.ItemId)
        and lower(c.ItemDataAreaId) = lower(e.dataAreaId)
        and lower(c.ItemGroupId) = lower('FU01')

    inner join SILVER_WAREHOUSE.dbo.ZSalesOrderHeader a
        on lower(a.SalesId) = lower(e.SalesId)
        and lower(a.ZSalesType) in (lower('FU'), lower('Return FU'))

    LEFT join SILVER_WAREHOUSE.dbo.DeviceModel DM
        on lower(c.ItemId) = lower(DM.ModelId)

    left join SILVER_WAREHOUSE.dbo.ZSalesOrderLine b
        on lower(b.SalesOrderNumber) = lower(a.SalesId)
        and lower(b.dataAreaId) = lower(a.dataAreaId)
        and lower(b.ItemNumber) = lower(e.ItemId)

    left join SILVER_WAREHOUSE.dbo.ZInventTables h
        on lower(h.ItemId) = lower(e.ItemId)
        and lower(h.dataAreaId) = lower(e.dataAreaId)

    left join SILVER_WAREHOUSE.dbo.ProductTranslation h1
        on lower(h1.Product) = lower(h.Product)

    left join SILVER_WAREHOUSE.dbo.SalesQuotationTable d
        on lower(d.SalesQuotationNumber) = lower(a.QuotationNumber)

    left join SILVER_WAREHOUSE.dbo.ZCustomers f
        on lower(f.AccountNum) = lower(a.ZLeasing)
        and lower(f.dataAreaId) = lower(a.dataAreaId)

    left join SILVER_WAREHOUSE.dbo.DirPartyTable ff
        on ff.RecordId = f.Party  -- numeric, tanpa LOWER()

    left join SILVER_WAREHOUSE.dbo.ZCustomers f0
        on lower(f0.AccountNum) = lower(a.CustAccount)
        and lower(f0.dataAreaId) = lower(a.dataAreaId)

    left join SILVER_WAREHOUSE.dbo.DirPartyTable f1
        on f1.RecordId = f0.Party  -- numeric, tanpa LOWER()

    left join SILVER_WAREHOUSE.dbo.DirPersonBaseEntity f2
        on lower(f2.PartyNumber) = lower(f1.PartyNumber)

    left join SILVER_WAREHOUSE.dbo.OpportunityTable g
        on lower(g.OpportunityId) = lower(d.OpportunityId)
        and lower(g.dataAreaId) = lower(d.dataAreaId)

    left join SILVER_WAREHOUSE.dbo.Dim i
        on lower(i.inventDimId) = lower(e.InventDimId)
        and lower(i.dataAreaId) = lower(e.dataAreaId)

    left join SILVER_WAREHOUSE.dbo.DeviceTableMasters j
        on lower(j.MasterId) = lower(i.InventDimension1)

    left Join SILVER_WAREHOUSE.dbo.DeviceTable k
        on lower(k.DeviceId) = lower(i.InventDimension1)
        and lower(k.dataAreaId) = lower(i.dataAreaId)

    left join SILVER_WAREHOUSE.dbo.DeviceGroup l
        on lower(l.DeviceGroupId) = lower(k.DeviceGroupId)
        and lower(l.dataAreaId) = lower(k.dataAreaId)

    left join (
        Select distinct x.*, b.OrderAccount, b.PurchName
        From (
        ...
        inner join SILVER_WAREHOUSE.dbo.PurchaseOrderHeaderV2 b
            on lower(b.PurchId) = lower(x.PurchaseOrderNumber)
            and lower(b.dataAreaId) = lower(x.dataAreaId)
    ) m1
        on lower(m1.InventRefId) = lower(e.SalesId)
        and left(lower(m1.ItemNumber), 4) in (lower('FU02'))

    left join SILVER_WAREHOUSE.dbo.VendTable m3
        on lower(m3.VendorAccountNumber) = lower(m1.OrderAccount)
        and lower(m3.dataAreaId) = lower(m1.dataAreaId)

    left join SILVER_WAREHOUSE.dbo.DirPartyPostalAddressView m4
        on m4.Party = m3.Party  -- numeric, tanpa LOWER()
        and m4.ValidTo >= getdate()
        and lower(m4.IsPrimary) = lower('Yes')

    left join SILVER_WAREHOUSE.dbo.ZCustInvoiceTrans n
        on lower(n.SalesId) = lower(e.SalesId)
        and left(lower(n.ItemId), 4) in (lower('FU02'))

    left join SILVER_WAREHOUSE.dbo.ZInventSites r
        on lower(r.SiteId) = lower(i.InventSiteId)

    left join SILVER_WAREHOUSE.dbo.ZClaimDASLine s
        on lower(s.ClaimId) = lower(e.SalesId)

    left join (
        select a.*
        from SILVER_WAREHOUSE.dbo.ZEFakpol a
        inner join (
            select VIN, min(RecordId) RecordId
            from SILVER_WAREHOUSE.dbo.ZEFakpol
            where ApprovedDate > '1900-01-01'
            group by VIN
        ) b
            on lower(b.VIN) = lower(a.VIN)
            and b.RecordId = a.RecordId  -- numeric, tanpa LOWER()
            and ApprovedDate > '1900-01-01'
    ) t
        on lower(t.SalesOrderNo) = lower(e.SalesId)
        and t.ApprovedDate > '1900-01-01'
        and lower(t.dataAreaId) = lower(e.dataAreaId)

    left join SILVER_WAREHOUSE.dbo.ZGoodReceiptSTNK u
        on lower(u.GoodReceiptSTNKNo) = lower(t.GRSTNKNo)

    left join SILVER_WAREHOUSE.dbo.ZGoodReceiptBPKB v
        on lower(v.GoodReceiptBPKB) = lower(t.GRBPKBNo)

    left join SILVER_WAREHOUSE.dbo.ZGoodIssueSTNK w
        on lower(w.GoodIssueSTNK) = lower(t.GISTNKNo)

    left join SILVER_WAREHOUSE.dbo.ZGoodIssueBPKB x
        on lower(x.GoodIssueBPKB) = lower(t.GIBPKBNo)

    left join SILVER_WAREHOUSE.dbo.ZEkspedisi y
        on lower(y.Code) = lower(a.ZEkspedisi)
        and lower(y.dataAreaId) = lower(a.dataAreaId)
    ...
    left join (
        ...
        on lower(z1.InventRefId) = lower(e.SalesId)
        and left(lower(z1.ItemNumber), 4) in (lower('FU04'))  -- vendor BBN Notice

    left join SILVER_WAREHOUSE.dbo.VendTable z3
        on lower(z3.VendorAccountNumber) = lower(z1.OrderAccount)
        and lower(z3.dataAreaId) = lower(z1.dataAreaId)

    left join SILVER_WAREHOUSE.dbo.DirPartyPostalAddressView z4
        on z4.Party = z3.Party  -- numeric, tanpa LOWER()
        and z4.ValidTo >= getdate()
        and lower(z4.IsPrimary) = lower('Yes')

    left join (
        Select RefRecId , sum(ZQty) ZQTY
        from SILVER_WAREHOUSE.dbo.smmQuotationCompetitors
        group by RefRecId
    ) aa
        on aa.RefRecId = g.RecordId  -- numeric, tanpa LOWER()

    left join SILVER_WAREHOUSE.dbo.smmBusRelTable ab
        on lower(ab.BusRelAccount) = lower(a.CustAccount)
        and lower(ab.dataAreaId) = lower(a.dataAreaId)
        and lower(ab.BusRelTypeId) = lower('Customer')

    left join SILVER_WAREHOUSE.dbo.Worker ac1
        on lower(ac1.PersonnelNumber) = lower(a.ZSupervisor)

    left join SILVER_WAREHOUSE.dbo.DirPartyTable ac2
        on ac2.RecordId = ac1.Person1  -- numeric, tanpa LOWER()

    left join SILVER_WAREHOUSE.dbo.Worker ad1
        on lower(ad1.PersonnelNumber) = lower(a.ZSalesman)

    left join SILVER_WAREHOUSE.dbo.DirPartyTable ad2
        on ad2.RecordId = ad1.Person1  -- numeric, tanpa LOWER()

    left join SILVER_WAREHOUSE.dbo.Ledger l1
        on lower(l1.Name) = lower(a.dataAreaId)

    left join SILVER_WAREHOUSE.dbo.TabelSDA z
        on lower(b.ZSDANo) = lower(z.SDANumber)

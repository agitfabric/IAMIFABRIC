...
BEGIN
	...
	SELECT  swo.data_customerAccount Customer_ID,  swo.data_name customer_Name,
			...
			Getdate() LastUpdate, 'AI' as DealerCategory, 
			---[dbo].name_masking_function(swo.data_name) MaskingName
			po.MaskedName as MaskingName,
			NULL as FlagDelete,NULL AS RecordId
	FROM SILVER_WAREHOUSE.dbo.service_workOrder swo 
		left join (select data_workOrderNo, data_parts_partNo, data_parts_partItemNo, data_parts_qty,
					...
					on LOWER(siw.data_workOrderNo) = LOWER(swo.data_workOrderNo) 
						and LOWER(siw.data_parts_partNo) = LOWER(swo.data_operations_parts_partNo)
		inner join SILVER_WAREHOUSE.dbo.site_mapping c on LOWER(c.SiteCodePSS) = LOWER(swo.data_site)
		inner join SILVER_WAREHOUSE.dbo.ZAISITES d on LOWER(d.SiteCode) = LOWER(c.SiteCode)
		CROSS APPLY SILVER_WAREHOUSE.dbo.name_masking_function(swo.data_name) as po
	WHERE swo.data_operations_parts_partNo is not null and swo.data_workOrderStatus is not null AND LOWER(swo.data_lineProperties) = 'billable'
	...
	Group by swo.data_customerAccount,  swo.data_workOrderNo,  swo.data_workOrderDateFrom,
		...
		swo.data_name, siw.data_invoiceNo, d.Dealer, d.DealerName, d.SiteCode, d.SiteName, d.Area,po.MaskedName
	...
	select a.data_customerAccount Customer_ID, a.data_name Customer_Name, 
			...
			Getdate() as Last_Update, 'AI' as DealerCategory, 
			---[dbo].name_masking_function(a.data_name)
			po.MaskedName as MaskingName,
			NULL as FlagDelete,NULL AS RecordId
	from SILVER_WAREHOUSE.dbo.sparepart_salesOrder a	
		left join SILVER_WAREHOUSE.dbo.sparepart_invoiceSO b on LOWER(b.data_salesOrderNo) = LOWER(a.data_salesOrderNo) and LOWER(b.data_items_item) = LOWER(a.data_items_itemNo)
		inner join SILVER_WAREHOUSE.dbo.site_mapping c on LOWER(c.SiteCodePSS) = LOWER(a.data_site)
		inner join SILVER_WAREHOUSE.dbo.ZAISITES d on LOWER(d.SiteCode) = LOWER(c.SiteCode)
		CROSS APPLY SILVER_WAREHOUSE.dbo.name_masking_function(a.data_name) as po
	where LOWER(a.data_items_itemGroup) = 'sp01' and LOWER(a.data_salesOrderStatus) != 'cancelled' and data_items_flagDeletion = 0
	...
	Group by a.data_customerAccount, a.data_name, a.data_salesOrderNo, a.data_pool, 
			...
			d.Dealer, d.DealerName, d.SiteCode, d.SiteName, d.Area,po.MaskedName
	)x

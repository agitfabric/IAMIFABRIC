CREATE PROCEDURE [dbo].[SP_REPORT_24]
AS
BEGIN
    ...
    -- Mulai loop per dataAreaId
       ...
       DELETE FROM Report_24 WHERE  TglFunneling BETWEEN @DateFrom AND @DateTo;
       ...
       INSERT INTO Report_24
        ...
        FROM SILVER_WAREHOUSE.dbo.OpportunityTable a
            ...
            LEFT JOIN (
    SELECT *,
           ROW_NUMBER() OVER (PARTITION BY Party ORDER BY ModifiedDate DESC) as rn
    FROM SILVER_WAREHOUSE.dbo.DirPartyPostalAddressView
    WHERE ValidTo >= DATEADD(HOUR,7,GETDATE())
) r ON r.Party = q.Party AND r.rn = 1
            ...
            LEFT JOIN (
    SELECT *,
           ROW_NUMBER() OVER (PARTITION BY AccountNum ORDER BY ModifiedDateTime1 DESC) as rn
    FROM SILVER_WAREHOUSE.dbo.ZCustomers
) z ON z.AccountNum COLLATE Latin1_General_CI_AS = b.CustAccount COLLATE Latin1_General_CI_AS 
   ...
   AND z.rn = 1
    ...
    FROM SILVER_WAREHOUSE.dbo.DirPartyPostalAddressView
    WHERE ValidTo >= DATEADD(HOUR,7,GETDATE())
) aa ON aa.Party = z.Party AND aa.rn = 1
         ...
         CAST(a.CreatedDateTime1 AS DATE) BETWEEN @DateFrom AND @DateTo and a.dataAreaId COLLATE Latin1_General_CI_AS <> 'kzu'
END